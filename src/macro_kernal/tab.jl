...
BroadcastStyle(::Type{TupleDummy{T,N,L,As,AXs}}) where {T,N,L,As,AXs} =
...
unalias(dest::TupleDummy, A::AbstractRange) = A
...
if isdefined(Base.Broadcast, :Scalar)
    Base.unalias(dest::TupleDummy, x::Broadcast.Scalar) = x
end
broadcast_unalias(dest::TupleDummy, src::AbstractArray) =
